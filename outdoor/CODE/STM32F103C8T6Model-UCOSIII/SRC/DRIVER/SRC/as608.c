...
{
//	PS_StaGPIO_Init();
	PS_GPIO(RCC_APB2Periph_GPIOA, GPIOA, GPIO_Pin_2);
	USART3_Init(57600);
}
...
/**
* @brief 合并特征(生成模板)
* 将CharBuffer1与CharBuffer2中的特征文件合并生成模板
...
uint8_t Add_FR(uint16_t id)
{
	...
	while(1)
	{
		switch(flag)
		{
			case 0:
				number++;
				//显示请按手指
				Clear_OLED_Zone(0, AS608_OLED_Y, 128, AS608_OLED_Y+2);
				PLEASE_FINGER(AS608_OLED_X, AS608_OLED_Y);
				LCD_SPI_Display_ALL();
				while(count--)
				{
					delay_ms(1);
				...
				if((ensure=PS_GenChar(CharBuffer1)) == 0x00)
				{
					//显示录入成功
					Clear_OLED_Zone(0, AS608_OLED_Y, 128, AS608_OLED_Y+2);
					ENTERY_SUCCESS(AS608_OLED_X, AS608_OLED_Y);
					LCD_SPI_Display_ALL();
					number = 0;
					flag = 1;
				}
				else
				{
					//错误
					Clear_OLED_Zone(0, AS608_OLED_Y, 128, AS608_OLED_Y+2);
					ENTERY_ERROR(AS608_OLED_X, AS608_OLED_Y);
					LCD_SPI_Display_ALL();
	//				return 1;
				}
	//			else
	...
	//			}
			...
			case 1:
				...
				Clear_OLED_Zone(0, AS608_OLED_Y, 128, AS608_OLED_Y+2);
				PLEASE_FINGER(AS608_OLED_X, AS608_OLED_Y);
	//			PLEASE_AGAN_FINGER(10, AS608_OLED_Y);
				LCD_SPI_Display_ALL();
				if((ensure=PS_GetImage()) == 0x00)
	...
	//				return 4;
			...
			case 4:
				...
				{
					Clear_OLED_Zone(0, AS608_OLED_Y, 128, AS608_OLED_Y+2);
					ENTER_OK(AS608_OLED_X, AS608_OLED_Y);
					LCD_SPI_Display_ALL();
					PS_ValidTempleteNum(&ValidN);//读库指纹个数
					//显示指纹个数
	//				OSTaskResume(&Press_FR_TCB, &err);
