...
extern Gozwits_Even_Mutex even_mutex;
/**
* @brief 事件处理接口
...
int8_t gizwitsEventProcess(eventInfo_t *info, uint8_t *data, uint32_t len)
{
  ...
  for(i=0; i<info->num; i++)
  {
    switch(info->event[i])
    {
      case EVENT_SWITCH_DOOR:
        currentDataPoint.valueswitch_door = dataPointPtr->valueswitch_door;
        GIZWITS_LOG("Evt: EVENT_SWITCH_DOOR %d \n", currentDataPoint.valueswitch_door);
        if(0x01 == currentDataPoint.valueswitch_door)
        {
          //user handle
//					LED_Open(0,1);
...
//					LED_Open(0,0);
        ...
        break;
      case EVENT_LATCH_LOCK:
        currentDataPoint.valuelatch_lock = dataPointPtr->valuelatch_lock;
        GIZWITS_LOG("Evt: EVENT_LATCH_LOCK %d \n", currentDataPoint.valuelatch_lock);
        if(0x01 == currentDataPoint.valuelatch_lock)
        {
          //user handle
					LED_Open(1,1);
					if (even_mutex.latch_lock == 0)
					{
						even_mutex.flag = 3;
						even_mutex.latch_lock = 1;
						even_mutex.delay_time = MOTOR_LOCK;
						OSTaskResume((OS_TCB*)&MOTORTCB, &err);
					}
        }
        else
        ...
        break;
      case EVENT_RANDOM_PASSWD:
        currentDataPoint.valuerandom_passwd = dataPointPtr->valuerandom_passwd;
        GIZWITS_LOG("Evt: EVENT_RANDOM_PASSWD %d \n", currentDataPoint.valuerandom_passwd);
        if(0x01 == currentDataPoint.valuerandom_passwd)
        {
          //user handle
        }
        else
        ...
        break;
      case EVENT_CHILDREN_MODE:
        currentDataPoint.valuechildren_mode = dataPointPtr->valuechildren_mode;
        GIZWITS_LOG("Evt: EVENT_CHILDREN_MODE %d \n", currentDataPoint.valuechildren_mode);
        if(0x01 == currentDataPoint.valuechildren_mode)
        {
          //user handle
        }
        else
      ...
      case EVENT_SWITCH_MODE:
        currentDataPoint.valueswitch_mode = dataPointPtr->valueswitch_mode;
        GIZWITS_LOG("Evt:EVENT_SWITCH_MODE %d\n",currentDataPoint.valueswitch_mode);
        //user handle
        break;
      case EVENT_VISITOR_PASSWD:
        currentDataPoint.valuevisitor_passwd = dataPointPtr->valuevisitor_passwd;
        GIZWITS_LOG("Evt:EVENT_VISITOR_PASSWD %d\n",currentDataPoint.valuevisitor_passwd);
        //user handle
        break;
