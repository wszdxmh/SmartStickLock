...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagswitch_door)
    {
        info->event[info->num] = EVENT_SWITCH_DOOR;
        info->num++;
        dataPoints->valueswitch_door = gizDecompressionValue(SWITCH_DOOR_BYTEOFFSET,SWITCH_DOOR_BITOFFSET,SWITCH_DOOR_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }



    if(0x01 == issuedData->attrFlags.flaglatch_lock)
    {
        info->event[info->num] = EVENT_LATCH_LOCK;
        info->num++;
        dataPoints->valuelatch_lock = gizDecompressionValue(LATCH_LOCK_BYTEOFFSET,LATCH_LOCK_BITOFFSET,LATCH_LOCK_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }



    if(0x01 == issuedData->attrFlags.flagrandom_passwd)
    {
        info->event[info->num] = EVENT_RANDOM_PASSWD;
        info->num++;
        dataPoints->valuerandom_passwd = gizDecompressionValue(RANDOM_PASSWD_BYTEOFFSET,RANDOM_PASSWD_BITOFFSET,RANDOM_PASSWD_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }



    if(0x01 == issuedData->attrFlags.flagchildren_mode)
    {
        info->event[info->num] = EVENT_CHILDREN_MODE;
        info->num++;
        dataPoints->valuechildren_mode = gizDecompressionValue(CHILDREN_MODE_BYTEOFFSET,CHILDREN_MODE_BITOFFSET,CHILDREN_MODE_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }



    if(0x01 == issuedData->attrFlags.flagswitch_mode)
    {
        info->event[info->num] = EVENT_SWITCH_MODE;
        info->num++;
        dataPoints->valueswitch_mode = gizX2Y(SWITCH_MODE_RATIO,  SWITCH_MODE_ADDITION, issuedData->attrVals.valueswitch_mode); 
    }



    if(0x01 == issuedData->attrFlags.flagvisitor_passwd)
    {
        info->event[info->num] = EVENT_VISITOR_PASSWD;
        info->num++;
        dataPoints->valuevisitor_passwd = gizX2Y(VISITOR_PASSWD_RATIO,  VISITOR_PASSWD_ADDITION, gizProtocolExchangeBytes(issuedData->attrVals.valuevisitor_passwd));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueswitch_door != cur->valueswitch_door)
    {
        GIZWITS_LOG("valueswitch_door Changed\n");
        ret = 1;
    }
    if(last->valuelatch_lock != cur->valuelatch_lock)
    {
        GIZWITS_LOG("valuelatch_lock Changed\n");
        ret = 1;
    }
    if(last->valuerandom_passwd != cur->valuerandom_passwd)
    {
        GIZWITS_LOG("valuerandom_passwd Changed\n");
        ret = 1;
    }
    if(last->valuechildren_mode != cur->valuechildren_mode)
    {
        GIZWITS_LOG("valuechildren_mode Changed\n");
        ret = 1;
    }
    if(last->valueswitch_mode != cur->valueswitch_mode)
    {
        GIZWITS_LOG("valueswitch_mode Changed\n");
        ret = 1;
    }
    if(last->valuevisitor_passwd != cur->valuevisitor_passwd)
    {
        GIZWITS_LOG("valuevisitor_passwd Changed\n");
        ret = 1;
    }
    if(last->valueFlag_switch != cur->valueFlag_switch)
    {
        GIZWITS_LOG("valueFlag_switch Changed\n");
        ret = 1;
    }

    if(last->valueswitch_time != cur->valueswitch_time)
    {
        if(gizGetTimerCount()-lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueswitch_time Changed\n");
            lastReportTime = gizGetTimerCount();
            ret = 1;
        }
    }
    if(last->valuedump_energy != cur->valuedump_energy)
    {
        if(gizGetTimerCount()-lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuedump_energy Changed\n");
            lastReportTime = gizGetTimerCount();
            ret = 1;
        }
    }
    if(last->valueswitch_manID != cur->valueswitch_manID)
    {
        if(gizGetTimerCount()-lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueswitch_manID Changed\n");
            lastReportTime = gizGetTimerCount();
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizCompressValue(SWITCH_DOOR_BYTEOFFSET,SWITCH_DOOR_BITOFFSET,SWITCH_DOOR_LEN,(uint8_t *)devStatusPtr,dataPoints->valueswitch_door);
    gizCompressValue(LATCH_LOCK_BYTEOFFSET,LATCH_LOCK_BITOFFSET,LATCH_LOCK_LEN,(uint8_t *)devStatusPtr,dataPoints->valuelatch_lock);
    gizCompressValue(RANDOM_PASSWD_BYTEOFFSET,RANDOM_PASSWD_BITOFFSET,RANDOM_PASSWD_LEN,(uint8_t *)devStatusPtr,dataPoints->valuerandom_passwd);
    gizCompressValue(CHILDREN_MODE_BYTEOFFSET,CHILDREN_MODE_BITOFFSET,CHILDREN_MODE_LEN,(uint8_t *)devStatusPtr,dataPoints->valuechildren_mode);
    gizCompressValue(FLAG_SWITCH_BYTEOFFSET,FLAG_SWITCH_BITOFFSET,FLAG_SWITCH_LEN,(uint8_t *)devStatusPtr,dataPoints->valueFlag_switch);
    ...
    devStatusPtr->valueswitch_mode = gizY2X(SWITCH_MODE_RATIO,  SWITCH_MODE_ADDITION, dataPoints->valueswitch_mode); 
    devStatusPtr->valueswitch_time = gizY2X(SWITCH_TIME_RATIO,  SWITCH_TIME_ADDITION, dataPoints->valueswitch_time); 
    devStatusPtr->valuedump_energy = gizY2X(DUMP_ENERGY_RATIO,  DUMP_ENERGY_ADDITION, dataPoints->valuedump_energy); 

    devStatusPtr->valuevisitor_passwd = gizProtocolExchangeBytes(gizY2X(VISITOR_PASSWD_RATIO,  VISITOR_PASSWD_ADDITION, dataPoints->valuevisitor_passwd)); 
    devStatusPtr->valueswitch_manID = gizProtocolExchangeBytes(gizY2X(SWITCH_MANID_RATIO,  SWITCH_MANID_ADDITION, dataPoints->valueswitch_manID)); 
