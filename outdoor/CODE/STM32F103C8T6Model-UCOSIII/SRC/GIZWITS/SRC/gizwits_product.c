...
dataSynchro synchro_data;
protocolTime_t ntptime;
...
int8_t gizwitsEventProcess(eventInfo_t *info, uint8_t *data, uint32_t len)
{
  ...
  for(i=0; i<info->num; i++)
  {
    switch(info->event[i])
    {
      case EVENT_SWITCH_ONOFF:
        currentDataPoint.valueswitch_onoff = dataPointPtr->valueswitch_onoff;
        GIZWITS_LOG("Evt: EVENT_SWITCH_ONOFF %d \n", currentDataPoint.valueswitch_onoff);
        if(0x01 == currentDataPoint.valueswitch_onoff)
        {
					Set_Temp(26);
          LED_Open(2,1);
        }
        ...
        break;
			...
			case EVENT_AIRLIGHT:
        currentDataPoint.valueairlight = dataPointPtr->valueairlight;
        GIZWITS_LOG("Evt: EVENT_AIRLIGHT %d \n", currentDataPoint.valueairlight);
        if(0x01 == currentDataPoint.valueairlight)
        {
          //user handle
        }
        else
      ...
      case EVENT_TEMP:
        currentDataPoint.valuetemp = dataPointPtr->valuetemp;
				synchro_data.temp = currentDataPoint.valuetemp;
        GIZWITS_LOG("Evt:EVENT_TEMP %d\n",currentDataPoint.valuetemp);
        //user handle
//				printf("\r\n valuetemp = %d\r\n",currentDataPoint.valuetemp);
				Set_Temp(synchro_data.temp);
//				printf("\r\n");
        break;

			case EVENT_OFF_TIME:
        currentDataPoint.valueoff_time = dataPointPtr->valueoff_time;
        GIZWITS_LOG("Evt:EVENT_OFF_TIME %4f\n",currentDataPoint.valueoff_time);
        //user handle
