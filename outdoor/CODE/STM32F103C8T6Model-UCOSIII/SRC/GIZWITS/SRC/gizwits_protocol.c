...
static int8_t gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagswitch_onoff)
    {
        info->event[info->num] = EVENT_SWITCH_ONOFF;
        info->num++;
        dataPoints->valueswitch_onoff = gizDecompressionValue(SWITCH_ONOFF_BYTEOFFSET,SWITCH_ONOFF_BITOFFSET,SWITCH_ONOFF_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }



    if(0x01 == issuedData->attrFlags.flagairlight)
    {
        info->event[info->num] = EVENT_AIRLIGHT;
        info->num++;
        dataPoints->valueairlight = gizDecompressionValue(AIRLIGHT_BYTEOFFSET,AIRLIGHT_BITOFFSET,AIRLIGHT_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
    ...
    if(0x01 == issuedData->attrFlags.flagoff_time)
    {
        info->event[info->num] = EVENT_OFF_TIME;
        info->num++;
        dataPoints->valueoff_time = gizX2YFloat(OFF_TIME_RATIO,  OFF_TIME_ADDITION, (float)(issuedData->attrVals.valueoff_time));
    }
...
//    static uint32_t lastReportTime = 0;
    ...
    if(last->valueswitch_onoff != cur->valueswitch_onoff)
    {
        GIZWITS_LOG("valueswitch_onoff Changed\n");
        ret = 1;
    }
    if(last->valueairlight != cur->valueairlight)
    {
        GIZWITS_LOG("valueairlight Changed\n");
        ret = 1;
    }
    ...
    if(last->valueoff_time != cur->valueoff_time)
    {
        GIZWITS_LOG("valueoff_time Changed\n");
        ret = 1;
    }
...
static int8_t gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizCompressValue(SWITCH_ONOFF_BYTEOFFSET,SWITCH_ONOFF_BITOFFSET,SWITCH_ONOFF_LEN,(uint8_t *)devStatusPtr,dataPoints->valueswitch_onoff);
    gizCompressValue(AIRLIGHT_BYTEOFFSET,AIRLIGHT_BITOFFSET,AIRLIGHT_LEN,(uint8_t *)devStatusPtr,dataPoints->valueairlight);
    ...
    devStatusPtr->valueoff_time = gizY2XFloat(OFF_TIME_RATIO,  OFF_TIME_ADDITION, dataPoints->valueoff_time); 
