...
void LED_Init(void)
{
	...
	GPIO_InitStructure1.GPIO_Pin = GPIO_Pin_14 | GPIO_Pin_15;//设置IO口
...
void LED_Open(uint8_t num, uint8_t status)
{
	if(status)
  {
    switch (num)
    {
      case 0: GPIO_SetBits(GPIOB, GPIO_Pin_14);break;//置1
      case 1: GPIO_SetBits(GPIOB, GPIO_Pin_15);break;//置1
    }
  }
	...
	else
  {
    switch (num)
    {
      case 0: GPIO_ResetBits(GPIOB, GPIO_Pin_14);break;//置0
      case 1: GPIO_ResetBits(GPIOB, GPIO_Pin_15);break;//置0
    }
  }
